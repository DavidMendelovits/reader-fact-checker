...
import { agentNdjson, type AgentContext } from './_impl.js'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { messages, context } = req.body as { messages: Parameters<typeof agentNdjson>[0]; context: AgentContext }
    if (!Array.isArray(messages) || messages.length === 0) return res.status(400).json({ error: 'messages required' })
    res.writeHead(200, { 'Content-Type': 'application/x-ndjson', 'Cache-Control': 'no-cache, no-transform' })
    await agentNdjson(messages, context, (line) => res.write(line))
    res.end()
  } catch (e) {
